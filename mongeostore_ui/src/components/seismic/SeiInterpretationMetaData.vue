 ...
 * @LastEditors: henggao
 * @LastEditTime: 2021-03-29 20:50:34
-->
<template>
  <div class="DataShow">
    <div>
            ...
            >
              <template slot-scope="scope">
                <el-button
                  ...
                  size="mini"
                  @click="
                    downloadfile(scope.row.seiInterpretation_filename, scope.row.id)
                  "
                  >下载文件</el-button
                >
              </template>
...
export default {
  ...
  methods: {
    ...
    downloadfile(seiInterpretation_filename, id) {
      console.log(id);
      const url = "http://127.0.0.1:8000/seismic/seiInterpretationfiledownload/";
      axios
        .get(url, {
          params: {
            file_id: id,
        ...
        .then((response) => {
          if (response.data.size === 0) {
            ...
            // link.download = seiInterpretation_filename + ".segy"; // 重命名文件
    ...
    deleteRow(index, rows, row) {
      ...
      this.$confirm("永久删除，是否继续？", "提示", {
        ...
        .then(() => {
          ...
          const url = "http://127.0.0.1:8000/seismic/deleteseiInterpretationinfo/";
          axios
            .post(
              url,
              {
    ...
    onSearchSubmit(n1, n2) {
      ...
      if (this.searchCondition.filter_key == "") {
        ...
        const url = "http://127.0.0.1:8000/seismic/seiInterpretationinfosearch/";
        axios
          .get(url, {
            params: {
              // 每页显示的条数
