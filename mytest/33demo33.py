...
version: v1.0.0
Author: henggao
Date: 2021-05-11 22:35:25
LastEditors: henggao
LastEditTime: 2021-05-12 09:56:04
ref:https://segmentfault.com/a/1190000011192866
未成功运行
'''
import requests
...
class mzitu():
    def all_url(self, url):
        ...
        all_a = BeautifulSoup(html.text, 'lxml').find(
            'div', class_='all').find_all('a', href=re.compile('[0~9]'))
    ...
    def mkdir(self, path):  # 创建文件夹
        ...
        if not isExists:
            ...
            os.makedirs(os.path.join("D:\mzitu2", path))
            os.chdir(os.path.join("D:\mzitu2", path))  # 切换到目录
            return True
        else:
            print(path, '文件夹已经存在了！')
            return False
